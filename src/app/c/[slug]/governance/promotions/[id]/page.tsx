...
export default function PromotionDetailPageScoped() {
  ...
  async function fetchPromotion() {
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Failed to fetch promotion");
  ...
  function getStatusColor(status: string) {
    switch (status) {
      ...
      case "expired": case "withdrawn": return "bg-gray-500/20 text-gray-600 border-gray-500/30";
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center"><div className="text-muted-foreground">Loading promotion...</div></div>;
  }

  if (error || !promotion) {
    return <div className="min-h-screen flex items-center justify-center"><div className="text-red-500">Error: {error || "Promotion not found"}</div></div>;
  }

  const votesNeeded = Math.ceil(promotion.total_eligible_voters * promotion.threshold);
  const progress = promotion.total_eligible_voters > 0 ? (promotion.votes_for_count / promotion.total_eligible_voters) * 100 : 0;
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {promotion.nominee_names.map((name, i) => (
                <span key={i} className="px-3 py-1 bg-accent/20 text-accent rounded-full text-sm">{name}</span>
              ))}
            </div>
          </div>
          {promotion.status === "pending" && <div className="text-sm text-muted-foreground">⏱ {getTimeRemaining(promotion.voting_ends_at)}</div>}
        </div>
        ...
        <div className="p-6 rounded-xl border border-border bg-background mb-8">
          ...
          <div className="mb-4">
            <div className="flex justify-between text-sm mb-1">
              <span><span className="text-green-600 font-semibold">{promotion.votes_for_count}</span> for / <span className="text-red-600 font-semibold">{promotion.votes_against_count}</span> against</span>
              <span className="text-muted-foreground">{votesNeeded} needed ({Math.round(promotion.threshold * 100)}% of {promotion.total_eligible_voters})</span>
            </div>
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            <div className="p-3 rounded-lg bg-muted/50 text-center"><div className="text-2xl font-bold text-green-600">{promotion.votes_for_count}</div><div className="text-muted-foreground">For</div></div>
            <div className="p-3 rounded-lg bg-muted/50 text-center"><div className="text-2xl font-bold text-red-600">{promotion.votes_against_count}</div><div className="text-muted-foreground">Against</div></div>
            <div className="p-3 rounded-lg bg-muted/50 text-center"><div className="text-2xl font-bold">{promotion.total_eligible_voters - promotion.votes_for_count - promotion.votes_against_count}</div><div className="text-muted-foreground">Remaining</div></div>
          </div>
        </div>
        ...
        <div className="p-6 rounded-xl border border-border bg-background">
          ...
          {votes.length === 0 ? (
            <div className="text-sm text-muted-foreground text-center py-4">No votes yet.</div>
          ) : (
            <div className="space-y-3">
              {votes.map((vote) => (
                <div key={vote.id} className="flex items-start justify-between p-3 rounded-lg bg-muted/30">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className={vote.vote ? "text-green-600" : "text-red-600"}>{vote.vote ? "✓" : "✗"}</span>
                      <span className="font-medium">{vote.voter_name}</span>
                    </div>
                    {vote.reason && <div className="text-sm text-muted-foreground mt-1 ml-5">&ldquo;{vote.reason}&rdquo;</div>}
                  </div>
                  <div className="text-xs text-muted-foreground">{new Date(vote.created_at).toLocaleString()}</div>
                </div>
              ))}
            </div>
          )}
