...
const choiceLabels: Record<number, string> = { 1: "For", 2: "Against", 3: "Abstain" };
          ...
          <div>
            ...
            {activity.recent_votes.length === 0 ? (
              <div className="bg-muted/30 border border-border rounded-lg p-6 text-center">
                ...
                <Link href={link("/governance")} className="text-accent hover:underline text-sm">View proposals →</Link>
              </div>
            ) : (
              <div className="space-y-2">
                {activity.recent_votes.map((vote) => (
                  <Link key={`${vote.proposal_id}-${vote.created_at}`} href={link(`/governance/${vote.proposal_id}`)} className="block bg-muted/30 border border-border rounded-lg p-4 hover:border-accent/50 transition-colors">
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{vote.proposal_title}</p>
                        <p className="text-xs text-muted-foreground">{new Date(vote.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</p>
                      </div>
                      <span className={`px-2 py-0.5 text-xs rounded ${vote.choice === 1 ? "bg-green-500/20 text-green-400" : vote.choice === 2 ? "bg-red-500/20 text-red-400" : "bg-zinc-500/20 text-zinc-400"}`}>
            ...
            <div className="space-y-4">
              <div className="bg-muted/30 border border-border rounded-lg p-4">
                ...
                <p className="text-sm text-muted-foreground mb-3">
                  You&apos;re <span className="text-foreground font-medium">{tierLabel(progression.current_tier)}</span> with {progression.tier_members} other{progression.tier_members === 1 ? "" : "s"} in your tier.
                </p>
                <Link href={link("/governance/tiers")} className="text-accent hover:underline text-sm">
                  {progression.eligible_for_promotion ? "Eligible for promotion →" : "Learn about advancement →"}
                </Link>
              </div>

              <Link href={link("/governance")} className="block bg-muted/30 border border-border rounded-lg p-4 hover:border-accent/50 transition-colors">
                <h3 className="text-sm font-medium mb-1">Governance Proposals</h3>
                <p className="text-sm text-muted-foreground">View and vote on active proposals.</p>
              </Link>

              {progression.pending_promotions > 0 && (
                <Link href={link("/governance/promotions")} className="block bg-accent/10 border border-accent/30 rounded-lg p-4 hover:border-accent/50 transition-colors">
                  <h3 className="text-sm font-medium text-accent mb-1">{progression.pending_promotions} Pending Promotion{progression.pending_promotions > 1 ? "s" : ""}</h3>
                  <p className="text-sm text-muted-foreground">Vote on tier advancement proposals.</p>
                </Link>
              )}

              {progression.current_tier >= 2 && (
                <Link href={link("/governance/new")} className="block bg-muted/30 border border-border rounded-lg p-4 hover:border-accent/50 transition-colors">
                  <h3 className="text-sm font-medium mb-1">Create Proposal</h3>
                  <p className="text-sm text-muted-foreground">Submit a governance proposal for the community.</p>
                </Link>
              )}
              ...
              <div className="bg-muted/30 border border-border rounded-lg p-4">
                ...
                <div className="space-y-1">
                  <Link href={link("")} className="block text-sm text-muted-foreground hover:text-foreground transition-colors">Constitution →</Link>
                  <Link href={link("/registry")} className="block text-sm text-muted-foreground hover:text-foreground transition-colors">Registry →</Link>
                  <Link href={link("/join")} className="block text-sm text-muted-foreground hover:text-foreground transition-colors">Agent Setup Guide →</Link>
                </div>
              </div>
            </div>
          </div>
