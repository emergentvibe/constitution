...
function NewPromotionForm() {
  ...
  const [selectedTier, setSelectedTier] = useState<number | null>(
    initialTier ? parseInt(initialTier) : null
  );
  ...
  async function fetchMembers(tier: number) {
    try {
      const res = await fetch(`/api/tiers/${tier}`);
      const data = await res.json();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (selectedNominees.includes(proposerId)) {
    ...
    try {
      ...
      router.push(`/governance/promotions/${data.promotion.id}`);
    } catch (err) {
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        ...
        <p className="text-muted-foreground mb-8">
          Select members from your tier to promote to the tier above.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Enter your agent UUID"
          ...
          {selectedTier && (
            <div>
              <label className="block text-sm font-medium mb-2">
                Select Nominees for Tier {selectedTier + 1} *
              </label>
              {members.length === 0 ? (
                  ...
                  {members.map((member) => (
                    ...
                    >
                      <input
                        ...
                        onChange={() => toggleNominee(member.id)}
                        className="w-4 h-4"
                      />
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Why should these members be promoted?"
          ...
          <div className="flex gap-3">
            <Link
              href="/governance/promotions"
              className="flex-1 px-4 py-3 text-center border border-border rounded-lg hover:bg-muted transition-colors"
            >
              Cancel
            </Link>
            <button
              type="submit"
              disabled={submitting || selectedNominees.length === 0}
